...
10. In settings.py:
    ...
    5. For any applications in the `ARCHES_APPLICATIONS` setting, reorder `INSTALLED_APPS` to place those before `"arches"` to preserve their higher preference with regard to templates and static files. Then, remove the `ARCHES_APPLICATIONS` setting.
    6. Replace:
        ```
