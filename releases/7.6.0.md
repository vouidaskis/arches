...
10. In settings.py:
    1. In `INSTALLED_APPS`:
        - remove `"compressor"`
        - add `"arches.app"`
        - Reorder anything also present in `ARCHES_APPLICATIONS` to a position before `"arches.app"` to preserve their higher preference with respect to templates and static files.
    ...
    5. Remove the `ARCHES_APPLICATIONS` setting.
    6. Replace:
        ```
